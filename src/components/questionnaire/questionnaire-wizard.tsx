...
export function QuestionnaireWizard({ initialStage = 0, invite = null }: { initialStage?: number; invite?: InviteState | null }) {
  ...
  const section = questionnaireSections[stage];
  const visibleQuestions = useMemo(() => section.questions.filter((question) => isQuestionVisible(question, answers)), [section, answers]);
  const progress = Math.round(((stage + 1) / questionnaireSections.length) * 100);
  ...
  function goForward() {
    ...
    const sectionErrors = validateSectionAnswers(section.id, normalized);
