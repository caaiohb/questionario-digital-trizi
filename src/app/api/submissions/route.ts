...
export async function POST(request: Request) {
  try {
    ...
    if (parsed.data.inviteToken) {
      const { error: inviteError } = await admin.from("questionnaire_invites").update({ status: "completed", completed_at: new Date().toISOString(), submission_id: inserted.id }).eq("token", parsed.data.inviteToken).eq("status", "pending");
      if (inviteError) console.error("[submissions] invite linking error:", JSON.stringify(inviteError));
    }
