...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  newPage(); text("QUESTIONÁRIO DIGITAL TRIZI", 19, bold, 5, rgb(0.14,0.27,0.24)); y -= 6; text(`Paciente: ${submission.patient_name}`, 12, bold); text(`Idade: ${submission.patient_age} anos`); text(`Data do preenchimento: ${format(new Date(submission.submitted_at), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}`); text(`Protocolo: ${submission.protocol}`); y -= 8; heading("Dados corporais"); text(`Peso atual: ${Number(submission.current_weight).toLocaleString("pt-BR")} kg`); text(`Peso desejado: ${Number(submission.desired_weight).toLocaleString("pt-BR")} kg`); text(`Altura: ${Number(submission.height).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m`);
